...
try:
    ...
    iwanttosleep = open("data/idk.txt", "w")
    iwanttosleep.write("")
try:
    open("data/token.txt", "r")
except FileNotFoundError:
    iwanttosleep = open("data/token.txt", "w")
try: # Creates a Try-Except construction.
    open("data/autostartbot.txt", "r") # Checks is there autostartbot.txt.
except FileNotFoundError: # If autostartbot does not exist this code will run.
    startfile = open("data/autostartbot.txt", "w") # Creates autobotstart.txt.
...
try: # Creates a Try-Except construction.
    open("data/language.txt", "r") # Checks is there language.txt.
except FileNotFoundError: # If language.txt not found, this code will run.
    checkfile = open("data/language.txt", "w")  # Creates the language.txt file.
...
with open("data/autostartbot.txt") as autocheck: # Opens autobotstart.txt.
    if "1" in autocheck.read(): # Searches for "1" in it (1=bot will start automatically).
        ...
        pass # Just pass.
with open("data/language.txt") as file: # Opens language.txt.
            ...
            if check=="2": # If its 2, this code will run.
                ...
                while True: # Cycle about token.
                    ...
                    if choice == "1": # If 1 is entered this code will run.
                        f = open("data/token.txt", "w") # Opens the settings.txt and deletes all previous data.
                        f.write(token) # Writes token in the file.
                        fidk = open("data/idk.txt", "w") # Opens the idk.txt and deletes all previous data.
                        fidk.write(idk) # Writes a new line in the file.
                        f.close() # Closes the file.
                        fidk.close()  # Closes the file.
                        okwell = open("data/settings.txt", "w")
                        okwell.close()
                        break # Breaks the cycle.
                    if choice == "2": # If 2, this one.
                        f = open("data/settings.txt", "w") # Deletes all from the settings.txt.
                ...
                while True: # Cycle about messanges.
                    ...
                    f = open("data/settings.txt", "a") # Opens settings.txt to add data.
...
if botison>1: # Checks the variable, if its bigger than 1, this code will run.
    try: # Makes a Try-Except construction.
        open("data/settings.txt", "r") # Checks is there settings.txt.
    except FileNotFoundError: # If settings.txt not found, this code will run.
    	with open("data/autostartbot.txt") as errorcheck: # Opens autostartbot.txt.
    ...
    f = open("data/token.txt", "r") # Open file to read it.
    lin = f.readlines() # Reads lines and saves it as a variable.
    fidk = open("data/idk.txt", "r") # Open file to read it.
    lin1 = fidk.readlines() # Reads lines and saves it as a variable.
    ...
    while True: # Bot cycle.
        try: # Makes a try cycle.
            ...
            if messages["count"] >= 1: # If amount of messages >=1, this code will run.
                ...
                with open("data/settings.txt") as file: # Opens settings.txt to read.
                    if word in file.read(): # Searches for the text of the message in the file.
                        with open("data/settings.txt", "rt") as file: # Open file again for reading, but in binary reading mode.
                            for index, line in enumerate(file): # Searches for index of the message.
                                if word in line: # If message text is in file, this code will run.
                                    help = open("data/settings.txt", "r") # Opens file again to read.
                                    ...
                                    if answer != "" and checkanswer / 2 % 2 != 0:
                                        ...
                                        vk.method("messages.send", {"peer_id": id, "message": idk, "random_id": random.randint(1, 2147483647)})
